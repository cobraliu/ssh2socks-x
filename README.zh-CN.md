...
- 显示连接卡在哪一步：日志会写明当前阶段（TCP 连接、SSH 版本交换、密钥交换、登录、经跳板机连接目标）、正在等哪台主机（跳板机还是服务器）以及可能原因；连接失败时也会记录断开前进行到哪一步，并记录 ssh 版本
- **SSH 配置管理**（「SSH 配置」页）：
...
- 中英文界面：包括报错、日志和托盘菜单。默认英文，可随时用右上角「EN / 中」按钮切换
...
- 使用**密钥免密登录**（以 `BatchMode=yes` 运行，不会弹出密码输入）；带口令的密钥请先用 `ssh-add` 加入 ssh-agent
